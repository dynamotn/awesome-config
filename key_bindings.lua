...
for i = 1, 9 do
  table.merge(list_global_keys['workspace'], {
    ...
    -- Toggle tag display.
    {
      ...
      'Toggle workspace #' .. i,
      function()
        action_workspace(i, awful.tag.viewtoggle)
      end,
    },
    -- Move client to tag.
    {
      ...
      function()
        if client.focus then
          action_workspace(i, function(tag)
    ...
    -- Toggle tag on focused client.
    {
      ...
      '#' .. i + 9,
      'Toggle focused window to workspace #' .. i,
      function()
        if client.focus then
          action_workspace(i, function(tag)
...
for group, list_group_keys in pairs(list_global_keys) do
  for _, group_keys in ipairs(list_group_keys) do
    table.merge(
      global_keys,
      awful.key(group_keys[1], group_keys[2], group_keys[4], { group = group, description = group_keys[3] })
    )
  end
end
