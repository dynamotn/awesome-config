...
local list_global_buttons = {
  ...
  ["window"] = {
    ...
    { { modkey }, 1, function(c) c:emit_signal("request::activate", "mouse_click", { raise = true }); awful.mouse.client.move(c) end },
    { { modkey }, 3, function(c) c:emit_signal("request::activate", "mouse_click", { raise = true }); awful.mouse.client.resize(c) end },
  },
  ["layout"] = {
  ...
  ["desktop"] = {
    { {        }, 3, function() main_menu:toggle() end },
...
local list_widget_buttons = {
  ["music"] = {
    { {        }, 1, function() misc.run_at_workspace(music_cmd, music_workspace) end }
  }
}
-- }
