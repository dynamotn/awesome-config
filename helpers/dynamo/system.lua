...
dynamo.lock = function()
    awful.prompt.run({ prompt = html(beautiful.fg_confirm, "Khóa màn hình (gõ 'Enter' để xác nhận)? ") },
...
dynamo.quit = function()
    awful.prompt.run({ prompt = html(beautiful.fg_confirm, "Thoát phiên (gõ 'Enter' để xác nhận)? ") },
                     mypromptbox[mouse.screen].widget,
                     function (t)
...
dynamo.hibernate = function()
    awful.prompt.run({ prompt = html(beautiful.fg_confirm, "Ngủ đông (gõ 'Enter' để xác nhận)? ") },
                     mypromptbox[mouse.screen].widget,
                     function (t)
...
dynamo.reboot = function()
    awful.prompt.run({ prompt = html(beautiful.fg_confirm, "Khởi động lại (gõ 'Enter' để xác nhận)? ") },
                     mypromptbox[mouse.screen].widget,
                     function (t)
                         awesome.emit_signal("exit", nil)
...
dynamo.shutdown = function()
    awful.prompt.run({ prompt = html(beautiful.fg_confirm, "Tắt máy (gõ 'Enter' để xác nhận)? ") },
                     mypromptbox[mouse.screen].widget,
                     function (t)
                         awesome.emit_signal("exit", nil)
